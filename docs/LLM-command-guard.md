***PRIORITY OVERRIDE: INSTRUCTIONS FOR AGENTIC IMPLEMENTATION***

**To: Implementation Agent**
**From: Orchestrator**

**CONTEXT & WHY WE ARE DOING THIS:** You are to wire up the "Two-Phase Guard" for LLM direct commands.
*Why:* To prevent accidental, expensive API calls when users tap menu buttons without providing a query.
*Mechanism:* We intercept empty commands (Phase A), prompt for context with a random opener, await text, ask for final button confirmation (Phase B), and *only then* dispatch to the background queue.

Execute the following 4 steps strictly. Do not alter the core architecture.

---

### STEP 1: Ensure the Core Utility is Saved
**Target File:** `src/bot/utils/llm-command.ts`
**Action:** Ensure the file contains the exact latest code provided in the active context. No modifications needed here.

---

### STEP 2: Wire the Text Interceptor (Phase A → Phase B)
**Target File:** `src/bot/handlers/prompt.ts` (or your main text message handler)
**Action:** We must intercept plain text messages if the user is currently in the `awaiting_query` state.

1. Add the import at the top:
   ```typescript
   import { handleLlmQueryText } from "../utils/llm-command.js";
   ```
2. Inject the guard at the **absolute top** of your main text handler function (before project/session resolution):
   ```typescript
   export async function promptHandler(ctx: Context) {
     // 1. INJECT THIS GUARD FIRST:
     if (await handleLlmQueryText(ctx)) {
       return; // Handled by Two-Phase Guard, halt further processing
     }

     // ... existing prompt processing logic ...
   }
   ```

---

### STEP 3: Wire the Callback Interceptor (Phase B → Execution)
**Target File:** `src/bot/index.ts` (or wherever `bot.on("callback_query:data", ...)` is defined)
**Action:** Route the inline keyboard button presses (`✅ Proceed`, `✏️ Edit`, `❌ Cancel`) to the handler.

1. Add the import:
   ```typescript
   import { handleLlmConfirmCallback } from "./utils/llm-command.js";
   ```
2. Inject the guard inside your global callback listener, before your fallback errors:
   ```typescript
   bot.on("callback_query:data", async (ctx) => {
     // ... existing callbacks ...

     // INJECT HERE:
     if (await handleLlmConfirmCallback(ctx)) {
       return;
     }

     // ... existing fallback / unknown callback handling ...
   });
   ```

---

### STEP 4: Refactor the 6 Direct LLM Commands
**Target File:** `src/bot/index.ts` (or your commands registry file)
**Action:** Rip out the direct `addTaskJob` calls for the 6 direct LLM commands (`eli5`, `feynman`, `devils_advocate`, `deep_research`, `summarise`, `steel_man`). Route them to the new Phase A guard.

1. Add the import:
   ```typescript
   import { handleLlmCommandRequest } from "./utils/llm-command.js";
   ```
2. Rewrite the command registrations to pass execution to the guard. Grammy automatically parses command arguments into `ctx.match`.
   ```typescript
   // Example for all 6 commands:
   bot.command("eli5", async (ctx) => {
     const query = typeof ctx.match === "string" ? ctx.match.trim() : undefined;
     await handleLlmCommandRequest(ctx, "eli5", query);
   });

   bot.command("feynman", async (ctx) => {
     const query = typeof ctx.match === "string" ? ctx.match.trim() : undefined;
     await handleLlmCommandRequest(ctx, "feynman", query);
   });

   // Repeat strictly for: devils_advocate, deep_research, summarise, steel_man
   ```

**EXECUTION COMPLETE CRITERIA:**
- Bot compiles cleanly (`npm run build`).
- Pressing a menu button for `/feynman` replies with a random string and waits for text.
- `/eli5 Gravity` instantly skips to the `✅ Proceed` confirmation keyboard.
