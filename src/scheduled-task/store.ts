...
async function mutateScheduledTasks<T>(
  ...
  const runMutation = async (): Promise<T> => {
    ...
    await setScheduledTasks(
      chatId,
      tasks.map((task) => cloneScheduledTask(task)),
    );
    return result;
  };

  const mutationPromise = scheduledTaskMutationQueue.then(
    runMutation,
    runMutation,
  );
...
export async function addScheduledTask(
  ...
  logger.info(
    `[ScheduledTaskStore] Added scheduled task: id=${task.id}, kind=${task.kind}`,
  );
}

export async function replaceScheduledTasks(
  chatId: number,
  tasks: ScheduledTask[],
): Promise<void> {
  ...
  logger.info(
    `[ScheduledTaskStore] Replaced scheduled task collection: count=${tasks.length}`,
  );
}
...
export async function updateScheduledTask(
  ...
  return mutateScheduledTasks(chatId, (tasks) => {
    ...
    const nextTask = cloneScheduledTask(
      updater(cloneScheduledTask(tasks[index])),
    );
