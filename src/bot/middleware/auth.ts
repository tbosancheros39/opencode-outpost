...
export async function authMiddleware(
  ...
  const isAuthorizedGroupChat =
    ...
    allowedChatIds.includes(ctx.chat.id);
  ...
  if (isAuthorizedUser || isAuthorizedGroupChat) {
    logger.debug(
      `[Auth] Access granted for userId=${userId}, chatId=${ctx.chat?.id}`,
    );
    await next();
  } else {
    // Silently ignore unauthorized users
    logger.warn(
      `Unauthorized access attempt from user ID: ${userId}, chat ID: ${ctx.chat?.id}`,
    );
    ...
    const isInAllowedChat =
      ctx.chat?.id && allowedChatIds.includes(ctx.chat.id);
    if (ctx.chat?.id && !isInAllowedChat) {
      try {
        ...
        logger.debug(
          `[Auth] Set empty commands for unauthorized chat_id=${ctx.chat.id}`,
        );
      } catch (err) {
        // Ignore errors
