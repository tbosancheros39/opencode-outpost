...
export function initializeTaskTracking(_bot: Bot<Context>): void {
  ...
  summaryAggregator.setOnComplete(async (sessionId, messageId, messageText) => {
    ...
    if (taskId) {
      logger.info(
        `[TaskTracking] Completing task ${taskId} for session ${sessionId}`,
      );
...
export function associateSessionWithTask(
  sessionId: string,
  taskId: string,
): void {
  activeTaskSessions.set(sessionId, taskId);
  logger.debug(
    `[TaskTracking] Associated session ${sessionId} with task ${taskId}`,
  );
}
...
export function recoverInterruptedTasks(): void {
  ...
  logger.warn(
    `[TaskTracking] Found ${runningTasks.length} interrupted tasks from previous session`,
  );
