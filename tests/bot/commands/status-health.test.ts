...
vi.mock("../../../src/monitoring/health-probes.js", () => ({
  checkRedisHealth: mocked.checkRedisHealthMock,
...
describe("bot/commands/status health display", () => {
  beforeEach(() => {
    mocked.healthMock.mockReset();
    mocked.checkRedisHealthMock.mockReset();
    mocked.checkOpencodeHealthMock.mockReset();
  ...
  it("shows Redis connected when Redis health returns ok", async () => {
  ...
  it("shows Redis down when Redis health returns not ok", async () => {
    mocked.checkRedisHealthMock.mockResolvedValue({
      ok: false,
